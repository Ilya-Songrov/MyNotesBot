...
class AbstractState : public QObject
{
    Q_OBJECT
public:
    explicit AbstractState(QObject *parent = nullptr);

    virtual void onAnyMessage(const Message::Ptr &message) = 0;
    virtual void onCallbackQuery(const CallbackQuery::Ptr &callbackQuery) = 0;
    virtual void saveMyState() = 0;
